package com.ascentya.AsgriV2.Utils;

import android.app.Dialog;
import android.content.Context;
import android.graphics.Color;
import android.graphics.drawable.ColorDrawable;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;
import android.view.Window;
import android.widget.ArrayAdapter;
import android.widget.Button;
import android.widget.EditText;
import android.widget.ImageView;
import android.widget.TextView;
import android.widget.Toast;

import com.androidnetworking.AndroidNetworking;
import com.androidnetworking.error.ANError;
import com.androidnetworking.interfaces.JSONObjectRequestListener;
import com.ascentya.AsgriV2.Models.LandCropModel;
import com.ascentya.AsgriV2.Models.Maincrops_Model;
import com.ascentya.AsgriV2.R;

import org.json.JSONArray;
import org.json.JSONObject;

import java.util.ArrayList;
import java.util.List;

import es.dmoral.toasty.Toasty;

public class Add_Income {
    NoDefaultSpinner crop_select, cropSpinner;
    EditText rows;
    Context contx;
    Button register;
    Dialog dialog;
    List<String> Data = new ArrayList<>();
    String land_id, user_i;
    ViewDialog viewDialog;
    String crop_name;
    Maincrops_Model model;
    String cropId, lcId;


    public void dialog(Context context, String title, String land_id, String crop_id, String user_i,
                       ViewDialog viewDialog, String crop_name, Maincrops_Model model, String lcId) {
        dialog = new Dialog(context, R.style.DialogSlideAnim);
        this.contx = context;
        this.land_id = land_id;
        this.model = model;

        this.user_i = user_i;
        this.viewDialog = viewDialog;
        this.crop_name = crop_name;

        this.cropId = crop_id;
        this.lcId = lcId;

        LayoutInflater factory = LayoutInflater.from(context);
        final View alertDialogView = factory.inflate(R.layout.add_incomedialog, null);


        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
//
        dialog.getWindow().setLayout(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.MATCH_PARENT);
        dialog.setContentView(alertDialogView);


        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));

        dialog.setCanceledOnTouchOutside(true);
        dialog.setCancelable(true);
        dialog.show();

        final TextView title_head = (TextView) dialog.findViewById(R.id.title);
        ImageView add_maincrop = (ImageView) dialog.findViewById(R.id.add_maincrop);
        ImageView close = (ImageView) dialog.findViewById(R.id.close);
        close.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                dialog.dismiss();
            }
        });
        crop_select = dialog.findViewById(R.id.crop_select);
        crop_select.setPrompt(contx.getString(R.string.income));
        rows = dialog.findViewById(R.id.rows);

        cropSpinner = dialog.findViewById(R.id.crop_list);

        rows.setHint("Amount");
        register = dialog.findViewById(R.id.register);
        register.setOnClickListener(new DebouncedOnClickListener(1500) {
            @Override
            public void onDebouncedClick(View v) {

                if (crop_select.getSelectedItemPosition() > -1 && (crop_id != null || cropSpinner.getSelectedItemPosition() > -1)) {

                    if (rows.getText().toString().length() > 0) {
                        String uniq = firstTwo(crop_select.getSelectedItem().toString()) + user_i + "-";
                        add_zone(uniq, crop_select.getSelectedItem().toString(),
                                crop_id == null ? ((LandCropModel) cropSpinner.getSelectedItem()).getCropId() :  crop_id,
                                lcId == null ? ((LandCropModel) cropSpinner.getSelectedItem()).getLcId() : lcId);
                    } else {
                        rows.setError(contx.getString(R.string.required));
                    }


                } else {
                    Toast.makeText(contx, "Kindly select Crop and Income Category", Toast.LENGTH_SHORT).show();
                }
            }
        });


        title_head.setText(title);
        get_crops();


        if(crop_id == null){
            setUpCropList();
        }
    }

    private void setUpCropList(){
        cropSpinner.setVisibility(View.VISIBLE);
        loadCrops();
    }

    private void loadCrops(){

        ArrayList<LandCropModel> landCrops = new ArrayList<>();

        landCrops.addAll(CropUtils.getLandCropModelList(model.getMainCropsString()));
        landCrops.addAll(CropUtils.getLandCropModelList(model.getInterCropsString()));

        ArrayAdapter<String> cropAdapter = new ArrayAdapter(contx, R.layout.spinner_item,
                landCrops);

        cropSpinner.setAdapter(cropAdapter);
    }

    private Integer searchFor(String data) {
        Integer pos = 0;

        //notifiy adapter
        for (int i = 0; i < Webservice.Data_crops.size(); i++) {
            String unitString = Webservice.Data_crops.get(i).getName().toLowerCase().trim();
            String C_name = Webservice.Data_crops.get(i).getS_name().toLowerCase().trim();


            if (unitString.equals(data.toLowerCase().trim()) || C_name.equals(data.toLowerCase().trim())) {


                pos = i;
                return pos;
            } else {
                pos = -1;
            }
        }
        return pos;
    }


    private Integer searchname(Integer data) {
        Integer pos = 0;

        //notifiy adapter
        for (int i = 0; i < Webservice.Data_crops.size(); i++) {
            Integer unitString = Integer.parseInt(Webservice.Data_crops.get(i).getCrop_id());


            if (unitString.equals(data)) {


                pos = i;
                return pos;
            } else {
                pos = -1;
            }
        }
        return pos;
    }

    public void add_zone(String uniq, String category, String cropId, String lcId) {
        viewDialog.showDialog();


        AndroidNetworking.post(Webservice.farmerincome)
                .addUrlEncodeFormBodyParameter("user_id", user_i)
                .addUrlEncodeFormBodyParameter("land_id", land_id)
                .addUrlEncodeFormBodyParameter("crop_id", cropId)
                .addUrlEncodeFormBodyParameter("lc_id", lcId)
                .addUrlEncodeFormBodyParameter("income_category", category)
                .addUrlEncodeFormBodyParameter("income", rows.getText().toString())
                .build().getAsJSONObject(new JSONObjectRequestListener() {

            @Override
            public void onResponse(JSONObject jsonObject) {
                viewDialog.hideDialog();

                if (UserHelper.checkResponse(contx, jsonObject)){
                    return;
                }


                try {
                    if (jsonObject.optString("status").equalsIgnoreCase("true")) {

                        dialog.dismiss();

                        Toasty.success(contx, jsonObject.optString("message"), Toast.LENGTH_SHORT, true).show();

                    } else {
                        Toasty.error(contx, jsonObject.optString("message"), Toast.LENGTH_SHORT, true).show();


                    }


                } catch (Exception e) {

                    e.printStackTrace();
                }

            }

            @Override
            public void onError(ANError anError) {
                viewDialog.hideDialog();


            }
        });


    }

    public void get_crops() {


        AndroidNetworking.get(Webservice.incomecat_get)

                .build().getAsJSONObject(new JSONObjectRequestListener() {
            @Override
            public void onResponse(JSONObject jsonObject) {

                if (UserHelper.checkResponse(contx, jsonObject)){
                    return;
                }


                try {
                    if (jsonObject.optString("status").equalsIgnoreCase("true")) {
                        JSONArray jsonArray = jsonObject.getJSONArray("data");

                        for (int i = 0; i < jsonArray.length(); i++) {

                            Data.add(jsonArray.getJSONObject(i).optString("name"));
                        }

                        ArrayAdapter<String> soiltype_adpter = new ArrayAdapter(contx, R.layout.spinner_item,
                                Data);

                        crop_select.setAdapter(soiltype_adpter);

                    } else {

                    }

                } catch (Exception e) {

                    e.printStackTrace();
                }

            }

            @Override
            public void onError(ANError anError) {

            }
        });


    }

    public String firstTwo(String str) {
        return str.length() < 2 ? str : str.substring(0, 2);
    }
}
