...
import com.ascentya.AsgriV2.AsyncTasks.phdControl_Async;
...
public class Phd_ControlMeasure_Fragment extends BaseFragment {

    private View root_view;
    private RecyclerView controlmeasure_recycler;
    private ControlMeasure_Adapter symtoms_adapter;
    private List<ControlMeasure_Model> Data;
        ...
        if (Webservice.Searchvalue.equalsIgnoreCase("none")) {
            ...
            GetSingleObject run = new GetSingleObject(getActivity(), new Async_Single<Info_Model>() {
                ...
                public void onTaskComplete(Info_Model result) {
                    ...
                    if (result != null && result.getPhd_controlmeasure() != null) {
                        loader_view.setVisibility(View.GONE);
                        Data.clear();
                        PControl_Converter con = new PControl_Converter();

                        Data.addAll(con.stringToSomeObjectList(result.getPhd_controlmeasure()));
    ...
    public void getsymt(final String lang) {
        AndroidNetworking.post(Webservice.getphdscontrolmeasure)
...
//                        obj.setCover_image(jsonArray.getJSONObject(i).optString("varieties_imgs"));
                        ...
                        JSONArray jsonArray1 = jsonArray.getJSONObject(i).getJSONArray("ph_disease_control_measures");
                        ...
                        if (jsonArray1.length() > 0) {
                            ...
                            for (int j = 0; j < jsonArray1.length(); j++) {
                                ...
                                String phd_control = jsonArray1.get(j).toString().trim();
                                if (!phd_control.equalsIgnoreCase("")) {
                                    controlmeasure.add(phd_control);
                                }
                    ...
                    if (Data.size() > 0) {
                        ...
                        phdControl_Async obj = new phdControl_Async(getActivity(), new AsyncTaskCompleteListener<String>() {
                            @Override
