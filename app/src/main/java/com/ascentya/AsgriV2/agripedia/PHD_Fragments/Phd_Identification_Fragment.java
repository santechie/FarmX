...
import com.ascentya.AsgriV2.AsyncTasks.PhdIdentification_Async;
...
public class Phd_Identification_Fragment extends BaseFragment {
    private View root_view;
    private RecyclerView identify_recycler;
    private Identification_Adapter symtoms_adapter;
    private List<Identification_Stages> Data;
        ...
        if (Webservice.Searchvalue.equalsIgnoreCase("none")) {
            ...
            GetSingleObject run = new GetSingleObject(getActivity(), new Async_Single<Info_Model>() {
                ...
                public void onTaskComplete(Info_Model result) {
                    if (result != null && result.getPhd_identification() != null) {
                        Data.clear();
                        PIdentification_Converter con = new PIdentification_Converter();

                        Data.addAll(con.stringToSomeObjectList(result.getPhd_identification()));

                        if (Data.size() > 0) {

                            title.setText(getString(R.string.phd_Identify) + " " + Webservice.Searchvalue);
    ...
    public void getsymt(final String lang) {
        AndroidNetworking.post(Webservice.getphdsidentification)
...
//                        obj.setCover_image(jsonArray.getJSONObject(i).optString("varieties_imgs"));
                        ...
                        JSONArray jsonArray1 = jsonArray.getJSONObject(i).getJSONArray("ph_disease_identify_pathogen");
                        JSONArray jsonArray2 = jsonArray.getJSONObject(i).getJSONArray("ph_disease_identify_pathogen_imgs");
                        ...
                        if (jsonArray1.length() > 0) {
                            ...
                            for (int j = 0; j < jsonArray1.length(); j++) {
                                ...
                                String phd_identify = jsonArray1.get(j).toString().trim();
                                if (!phd_identify.equalsIgnoreCase("")) {
                                    controlmeasure.add(phd_identify);
                                }
                    ...
                    if (Data.size() > 0) {
                        ...
                        PhdIdentification_Async obj = new PhdIdentification_Async(getActivity(), new AsyncTaskCompleteListener<String>() {
                            @Override
