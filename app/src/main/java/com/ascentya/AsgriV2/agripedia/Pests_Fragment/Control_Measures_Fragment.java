...
import com.ascentya.AsgriV2.AsyncTasks.ControlMeasure_Async;
...
public class Control_Measures_Fragment extends BaseFragment {
    View root_view;
        ...
        if (Webservice.Searchvalue.equalsIgnoreCase("none")) {
            ...
            GetSingleObject run = new GetSingleObject(getActivity(), new Async_Single<Info_Model>() {
                ...
                public void onTaskComplete(Info_Model result) {
                    ...
                    if (result != null && result.getPests_controlmeasure() != null) {
                        loader_view.setVisibility(View.GONE);
                        Data.clear();
                        PControl_Converter con = new PControl_Converter();

                        Data.addAll(con.stringToSomeObjectList(result.getPests_controlmeasure()));
    ...
    public void getsymt(final String lang) {
        AndroidNetworking.post(Webservice.getpestscontrolmeasure)
...
//                        obj.setCover_image(jsonArray.getJSONObject(i).optString("varieties_imgs"));
                        ...
                        JSONArray jsonArray1 = jsonArray.getJSONObject(i).getJSONArray("pd_pest_control_measures");


                        if (jsonArray1.length() > 0) {
                            List<String> controlmeasure = new ArrayList<>();
                            for (int j = 0; j < jsonArray1.length(); j++) {
                                String symtoms_measure = jsonArray1.get(j).toString().trim();

                                if (!symtoms_measure.equalsIgnoreCase("")) {
                                    controlmeasure.add(symtoms_measure);
                                }
                    ...
                    if (Data.size() > 0) {
                        ...
                        ControlMeasure_Async obj = new ControlMeasure_Async(getActivity(), new AsyncTaskCompleteListener<String>() {
                            @Override
