...
import com.ascentya.AsgriV2.AsyncTasks.DiseaseIdentification_Async;
...
public class Identification_Fragment extends BaseFragment {
        ...
        if (Webservice.Searchvalue.equalsIgnoreCase("none")) {
            ...
            GetSingleObject run = new GetSingleObject(getActivity(), new Async_Single<Info_Model>() {
                ...
                public void onTaskComplete(Info_Model result) {
                    if (result != null && result.getDiseas_identification() != null) {
                        Data.clear();
                        PIdentification_Converter con = new PIdentification_Converter();

                        Data.addAll(con.stringToSomeObjectList(result.getDiseas_identification()));

                        if (Data.size() > 0) {
                            title.setText(getString(R.string.Disease_Identify) + " " + Webservice.Searchvalue);
    ...
    public void getsymt(final String lang) {
        AndroidNetworking.post(Webservice.getdideasidentification)
...
//                        obj.setCover_image(jsonArray.getJSONObject(i).optString("varieties_imgs"));
                        ...
                        JSONArray jsonArray1 = jsonArray.getJSONObject(i).getJSONArray("pd_disease_identify_pathogen");
                        JSONArray jsonArray2 = jsonArray.getJSONObject(i).getJSONArray("pd_disease_identify_images");


                        if (jsonArray1.length() > 0) {
                            List<String> controlmeasure = new ArrayList<>();
                            for (int j = 0; j < jsonArray1.length(); j++) {
                                String diseas_identify = jsonArray1.get(j).toString().trim();

                                if (!diseas_identify.equalsIgnoreCase("")) {
                                    controlmeasure.add(diseas_identify);
                                }
                    ...
                    if (Data.size() > 0) {
                        ...
                        DiseaseIdentification_Async obj = new DiseaseIdentification_Async(getActivity(), new AsyncTaskCompleteListener<String>() {
                            @Override
